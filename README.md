# CampusPe | Python Basics Assignment

## Modules 1-6: Basics to Functions

**Name:** [Your Name]
**Questions Attempted:** [Q1-Q20]

### Assignment Instructions

1. Test your code with multiple inputs before submission.
2. Add comments explaining your logic throughout your code.
3. Use meaningful and descriptive variable names.
4. Handle edge cases and invalid inputs using try-except where appropriate.
5. Include a README.txt with your name, questions attempted, special instructions, and challenges faced.

### Grading Rubric

- Code Functionality (60%): Correct Output (30%), Edge Cases (15%), Efficiency (15%)
- Code Quality (25%): Readability (10%), Comments (8%), Variable Names (7%)
- Problem Understanding (15%): Requirements Met (10%), Completeness (5%)

### Special Instructions

[Add any special instructions here]

### Challenges Faced

[Add challenges faced here]
